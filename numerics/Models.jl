...
function BilayerLEEReal(
    ...
    for (i, l) in enumerate(layerSpecs)
        bath_i = 3 + 2 * i
        for t in ["f", "d"]
            if l == t && bath_i + 1 + 2 * hop_step[t] < 2 * (2 + length(layerSpecs))
                push!(hamiltonian, ("+-",  [bath_i, bath_i + 2 * hop_step[t]], -hop_t[t]))
                push!(hamiltonian, ("+-",  [bath_i + 2 * hop_step[t], bath_i], -hop_t[t]))
                push!(hamiltonian, ("+-",  [bath_i + 1, bath_i + 1 + 2 * hop_step[t]], -hop_t[t]))
                push!(hamiltonian, ("+-",  [bath_i + 1 + 2 * hop_step[t], bath_i + 1], -hop_t[t]))
            end
        end
    end

function SpinCorr(i, j, k)
    spinCorr = Tuple{String, Vector{Int64}, Float64}[]
    push!(spinCorr, ("n+-", [i, j, k], 0.25))
    push!(spinCorr, ("n+-", [i, j+1, k+1], -0.25))
    push!(spinCorr, ("n+-", [i+1, j, k], -0.25))
    push!(spinCorr, ("n+-", [i+1, j+1, k+1], 0.25))
    push!(spinCorr, ("+-+-", [i, i+1, j+1, k], 0.5))
    push!(spinCorr, ("+-+-", [i+1, i, j, k+1], 0.5))
    return spinCorr
end
